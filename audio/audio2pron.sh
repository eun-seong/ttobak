...
$KALDI_ROOT/src/latbin/lattice-align-phones --replace-output-symbols=true $exp/tdnn1n_rvb_online/final.mdl "ark:gunzip -c ${dest}/lat.1.gz|" ark:$dest/phone_aligned.lats
...
$KALDI_ROOT/src/bin/show-alignments $exp/tree_a/graph_tgsmall/phones.txt $exp/tdnn1n_rvb_online/final.mdl ark:$dest/out.ali >> $dest/temp.txt

# 파일을 phone sequence로 변환
python3 local/result2phone.py $dest/temp.txt $dest/result.txt

# 발음 정확도 계산
python3 local/calc_pron_score.py $dest/result.txt $data/$course/${course}.prons.txt $final/${course}_${user}_${id}.txt
